...
public class MediaPanel extends Application {
    ...
    public static android.app.Notification PushNotification(Context context, int REQ_CODE, int PlayPauseCode, String textPlay, MediaSessionCompat.Token token) {
        ...
        Intent PlayPause = new Intent(context, mediaService.class);
        PlayPause.setAction("PLAY");
        PendingIntent PlayPending = PendingIntent.getService(context, REQ_CODE, PlayPause, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);

        /*Next Play ACTION*/
        Intent NextPlay = new Intent(context, mediaService.class);
        NextPlay.setAction("NEXT");
        PendingIntent NextPending = PendingIntent.getService(context, REQ_CODE, NextPlay, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);

        /*Previous Play ACTION*/
        Intent PrevPlay = new Intent(context, mediaService.class);
        PrevPlay.setAction("PREVIOUS");
        PendingIntent PrevPending = PendingIntent.getService(context, REQ_CODE, PrevPlay, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);

        Intent ClosePlay = new Intent(context, mediaService.class);
        ClosePlay.setAction("CLOSE");
        PendingIntent ClosePending = PendingIntent.getService(context, REQ_CODE, ClosePlay, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(context, CHANNEL_ID).setLargeIcon(image).setSmallIcon(R.drawable.quran_icon).setSilent(true).setVisibility(NotificationCompat.VISIBILITY_PUBLIC).setAutoCancel(true).setDefaults(NotificationCompat.PRIORITY_HIGH).setCategory(NotificationCompat.EXTRA_MEDIA_SESSION).setContentIntent(pendingIntent).addAction(R.drawable.ic_previous, "Previous", PrevPending).addAction(PlayPauseCode, textPlay, PlayPending)
                ...
                .addAction(R.drawable.ic_next, "Next", NextPending).addAction(R.drawable.ic_close, "Close", ClosePending).setStyle(new androidx.media.app.NotificationCompat.MediaStyle().setShowCancelButton(true).setCancelButtonIntent(ClosePending).setShowActionsInCompactView(0, 1, 2).setMediaSession(token)).setOngoing(true);
